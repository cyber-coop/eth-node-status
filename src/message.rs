...
pub fn create_status_message(version: &usize, genesis_hash: &Vec<u8>, head_hash: &Vec<u8>, head_td: &u64, fork_id: &Vec<u32>, network_id: &u32) -> Vec<u8> {
...
pub fn create_hello_message(private_key: &Vec<u8>) -> Vec<u8> {
    ...
    let hello = types::HelloMessage {
        ...
        // We lie and tell people we are geth node
        // client_version: String::from("Geth/v1.13.2-stable-dc34fe82/linux-amd64/go1.21.1"),
        capabilities: vec![
            types::CapabilityMessage{ name: types::CapabilityName(ArrayString::from("eth").unwrap()), version: 64 },
            types::CapabilityMessage{ name: types::CapabilityName(ArrayString::from("eth").unwrap()), version: 65 },
            types::CapabilityMessage{ name: types::CapabilityName(ArrayString::from("eth").unwrap()), version: 66 },
            types::CapabilityMessage{ name: types::CapabilityName(ArrayString::from("eth").unwrap()), version: 67 },
            types::CapabilityMessage{ name: types::CapabilityName(ArrayString::from("eth").unwrap()), version: 68 }],
        port: 0,
        id: primitive_types::H512::from_slice(&secp256k1::PublicKey::from_secret_key(&secp,&private_key).serialize_uncompressed()[1..]),
    };
